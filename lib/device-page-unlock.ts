...
export function getDevicePageUnlockCookieName(slug: string): string {
  const safe = slug.replace(/[^a-zA-Z0-9_-]/g, "_").slice(0, 48);
  return `device_page_unlock_${safe}`;
}

export async function createDevicePageUnlockToken(
  slug: string,
  passwordHash: string
): Promise<string> {
...
export async function verifyDevicePageUnlockToken(
...
export function getDevicePageUnlockCookieOptions() {
  return {
...
export async function isDevicePageUnlocked(
  slug: string,
  passwordHash: string | null | undefined
): Promise<boolean> {
  if (!passwordHash) return true;
  const cookieStore = await cookies();
  const token = cookieStore.get(getDevicePageUnlockCookieName(slug))?.value;
  return verifyDevicePageUnlockToken(token, slug, passwordHash);
}
